-- LGE pilot schema — plain PostgreSQL (self-hosted via Coolify + PostgREST)
-- Same data model as supabase-schema.sql, RLS off for pilot (dashboard unlisted;
-- add auth before public exposure). gen_random_uuid() is core in PG13+.
\set ON_ERROR_STOP on

create table if not exists clients (
  id uuid primary key default gen_random_uuid(),
  name text not null, business text, niche text, city text default 'Kanpur',
  phone text, email text, access_code text unique,
  ...
  started_at timestamptz default now(), notes text, created_at timestamptz default now());

create table if not exists leads (
  id uuid primary key default gen_random_uuid(),
  name text, business text, niche text, city text default 'Kanpur', phone text, email text,
  stage text check (stage in ('new','contacted','replied','probing','demo_sent','negotiating','closed_won','closed_lost')) default 'new',
  source text default 'manual', score int default 0, next_action text, next_action_at timestamptz,
  odoo_id text, tags text[] default '{}', notes text, is_test boolean default false,
...
create table if not exists messages (
  ...
  body text, chunks jsonb default '[]', wa_id text,
...
create table if not exists outreach_log (
  id uuid primary key default gen_random_uuid(),
  lead_id uuid references leads(id) on delete cascade,
  template text, variant int default 1, day_no int default 1, channel text default 'whatsapp',
  status text check (status in ('sent','failed','skipped')) default 'sent', detail text,
  created_at timestamptz default now());
...
create table if not exists demos (
  ...
  title text, asset_url text, feedback text, sent_at timestamptz default now());

create table if not exists payments (
  id uuid primary key default gen_random_uuid(),
  lead_id uuid references leads(id) on delete set null,
  client_id uuid references clients(id) on delete set null,
  amount numeric not null, currency text default 'INR',
  type text check (type in ('trial','annual','placard','setup','upsell')) default 'trial',
  gateway_ref text, status text check (status in ('link_sent','paid','failed','refunded')) default 'link_sent',
  created_at timestamptz default now());
...
create table if not exists escalations (
  ...
  detail text, status text check (status in ('open','handled')) default 'open',
...
create table if not exists analytics_daily (
  ...
  reviews_received int default 0, reviews_replied int default 0, avg_review_rating numeric(2,1),
  blogs_published int default 0, posts_published int default 0, assets_delivered int default 0,
  traffic_users int, leads_captured int default 0, notes jsonb, created_at timestamptz default now(),
  unique (client_id, day));

create table if not exists credentials (
  id uuid primary key default gen_random_uuid(),
  client_id uuid references clients(id) on delete cascade,
  label text not null, secret_enc text not null,
  created_by text default 'owner', last_used_at timestamptz, created_at timestamptz default now());

create table if not exists events (
  id uuid primary key default gen_random_uuid(),
  type text not null, lead_id uuid references leads(id) on delete set null,
  client_id uuid references clients(id) on delete set null,
...
-- RPCs (PostgREST exposes these as /rpc/<name> — portal-compatible as-is)
...
returns table (day date, reviews_received int, reviews_replied int, avg_review_rating numeric,
               blogs_published int, posts_published int, assets_delivered int, traffic_users int, leads_captured int)
language sql security definer set search_path = public as $$
  select a.day, a.reviews_received, a.reviews_replied, a.avg_review_rating, a.blogs_published,
         a.posts_published, a.assets_delivered, a.traffic_users, a.leads_captured
  from analytics_daily a join clients c on c.id = a.client_id
...
language sql security definer set search_path = public as $$
  ...
  where c.access_code = p_access order by a.created_at desc limit 100;
$$;
...
language sql security definer set search_path = public as $$
  ...
  where c.access_code = p_access order by t.created_at desc limit 50;
$$;
...
-- PostgREST anon role
do $$ begin
  if not exists (select 1 from pg_roles where rolname='lge_anon') then
    create role lge_anon nologin;
  end if;
end $$;
grant usage on schema public to lge_anon;
grant select, insert, update, delete on all tables in schema public to lge_anon;
grant execute on all functions in schema public to lge_anon;
alter default privileges in schema public grant all on tables to lge_anon;
alter default privileges in schema public grant execute on functions to lge_anon;
-- NOTE: credentials table intentionally NOT granted to lge_anon (service-role only). Grant it manually if ever needed:
-- grant select, update on credentials to lge_anon;
...
values ('Test Lead - Dental Wellness', 'Dental Wellness Clinic', 'dentist', 'Kanpur',
        '917705871046', 'contacted', 'test', true,
        'Owner-run pilot drill. D1 outreach sent 29-Aug-2026 via Evolution (5 chunks). Awaiting reply.')
